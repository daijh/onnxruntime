...
ARM64EC, and WindowsStore/UWP targets are not supported. Python wheels include the required D3D12 runtime DLLs. C#,
NuGet, Java, and Node.js packages are not supported because they do not deploy those DLLs. Custom Dawn checkouts selected with
...
At runtime, Dawn loads the SDK DLLs from the `D3D12` directory next to the Dawn module. In a Python wheel this is
`onnxruntime/capi/D3D12`. Look for `[AgilitySDK] active` in the Dawn log to confirm successful activation.
